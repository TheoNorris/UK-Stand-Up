...
function artistInformationHTML(id_artist) {
    ...
    /*For Loop for appending and removing social media loops*/
    for (i = 0, j = 0; i < socialNames.length, j < socialLinks.length && i < 4; i++, j++) {
        if (socialLinks[j] != "") {
            socialMediaLinks += `<a id="${socialNames[i]}" class="social-links" href="https://www.${socialNames[i]}.com/${socialLinks[j]}" target="_blank"><i class="fab fa-${socialNames[i]}-square"></i></a>`;
            var socialMediaToDiv = socialMediaLinks.replace("undefined", "");
...
function artistDescriptionHTML(artist) {
    return $("#artist-description-p").html(artist.artist.bio.summary), $("#artist-description-p").children().css("color", "#3f83ff").attr("target", "_blank");
}
    ...
    $.when(
        ...
        function (errorResponse) {
            if (errorResponse.status === 404) {
                ...
                $("#artist-information").html(`<h2>Error: ${errorResponse.responseJSON.message}</h2>`);
            }
        }
    );
}
